...
def main():
    ...
    st.write('新規枠譲渡申請をする')
    new_transfer_name = st.selectbox('漁業者名を選択してください', name_list, key = 'new_transfer_name') # 将来的にはDBもしくはconfigから取得する
    ...
    if new_transfer_submit:
        ...
        formatted_date = datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=9))).strftime('%Y/%m/%d')
    ...
    st.write('新規枠取得申請をする')
    new_get_name = st.selectbox('漁業者名を選択してください', name_list, key = 'new_get_name') # 将来的にはDBもしくはconfigから取得する
